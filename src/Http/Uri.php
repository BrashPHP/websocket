...
{
    public const int REQUIRE_PORT = 1; // Always include port, explicit or default
    public const int ABSOLUTE_PATH = 2; // Enforce absolute path
    public const int NORMALIZE_PATH = 4; // Normalize path
    public const int IDNA = 8; // @deprecated, replaced by IDN_ENCODE
    public const int IDN_ENCODE = 16; // IDN-encode host
    public const int IDN_DECODE = 32; // IDN-decode host
    public const int URI_DECODE = 64; // Decoded URI
    public const int URI_ENCODE = 128; // Minimal URI encoded
    public const int URI_ENCODE_3986 = 256; // URI encoded RFC 3986

    private const string RE_MAIN = '!^(?P<schemec>(?P<scheme>[^:/?#]+):)?(?P<authorityc>//(?P<authority>[^/?#]*))?'
        . '(?P<path>[^?#]*)(?P<queryc>\?(?P<query>[^#]*))?(?P<fragmentc>#(?P<fragment>.*))?$!';
    private const string RE_AUTH = '!^(?P<userinfoc>(?P<user>[^:/?#]+)(?P<passc>:(?P<pass>[^:/?#]+))?@)?'
        . '(?P<host>[^:/?#]*|\[[^/?#]*\])(?P<portc>:(?P<port>[0-9]*))?$!';

    const array PORTS_DEFAULT  = [
        'acap' => 674,
        'afp' => 548,
        'dict' => 2628,
        'dns' => 53,
    ...
    public function getHost(int $flags = 0): string
    {
        ...
        if ($flags & self::IDNA) {
            ...
            $host = $this->idnEncode($this->host);
        }
        if ($flags & self::IDN_ENCODE) {
            $host = $this->idnEncode($this->host);
        }
        if ($flags & self::IDN_DECODE) {
            $host = $this->idnDecode($this->host);
        }
        return $host;
    }
    ...
    {
        $default = self::PORTS_DEFAULT[$this->scheme] ?? null;
    ...
    public function getPath(int $flags = 0): string
    {
        ...
        if ($flags & self::ABSOLUTE_PATH && !str_starts_with($path, '/')) {
            $path = "/{$path}";
        }
    ...
    public function withComponents(array $components, int $flags = 0): UriInterface
    {
        ...
        foreach ($components as $component => $value) {
            match ($component) {
                'port' => $clone->setPort($value, $flags),
                'scheme' => $clone->setScheme($value, $flags),
                'host' => $clone->setHost($value, $flags),
                'path' => $clone->setPath($value, $flags),
                'query' => $clone->setQuery($value, $flags),
                'fragment' => $clone->setFragment($value, $flags),
                'userInfo' => $clone->setUserInfo(...$value),
                default => throw new InvalidArgumentException("Invalid URI component: '{$component}'"),
            };
        }
        return $clone;
    }
    ...
    private function parse(string $uri_string = ''): void
{
    ...
    $this->setScheme($main['schemec'] ?? '');
    $this->setPath($main['path'] ?? '');
    $this->setQuery($main['queryc'] ?? '');
    $this->setFragment($main['fragmentc'] ?? '');

    if ($this->authority) {
        $this->parseAuthority($main['authority']);
    }
}

private function parseAuthority(string $authority): void
{
    preg_match(self::RE_AUTH, $authority, $auth);

    if (empty($auth) && $authority !== '') {
        throw new InvalidArgumentException("Invalid 'authority'.");
    }

    if ($auth['host'] === '' && !empty($auth['user'])) {
        throw new InvalidArgumentException("Invalid 'authority'.");
    }

    $this->setUser($auth['user'] ?? '');
    $this->setPassword($auth['passc'] ?? null);
    ...
    private function uriEncode(string $source, int $flags = 0, string $keep = ''): string
    {
        ...
        return preg_replace_callback($re, fn($matches) => rawurlencode((string) $matches[0]), $source);
    }

    private function uriDecode(string $source): string
    {
        $re = "/(%[A-Fa-f0-9]{2})/u";
        return preg_replace_callback($re, fn($matches) => rawurldecode((string) $matches[0]), $source);
    }
